...
namespace BeautyPlanet.DataAccess
{
    public class BeautyDbContext : IdentityDbContext<Person>
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Product>().HasMany(s => s.Sizes).WithMany(p => p.Products).UsingEntity<ProductColorSize>(
                pc => pc.HasOne(prop => prop.Size).WithMany().HasForeignKey(prop => prop.SizeId),
                pc => pc.HasOne(prop => prop.Productt).WithMany().HasForeignKey(prop => prop.ProductId),
                pc => pc.HasIndex(prop => new { prop.ProductId, prop.SizeId }));
            modelBuilder.Entity<Product>().HasMany(s => s.Colors).WithMany(p => p.Products).UsingEntity<ProductColorSize>(
